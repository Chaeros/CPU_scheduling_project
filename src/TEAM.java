...
public class TEAM {
	...
	int preemptive; // 선점, 비선점 스케줄링 정책(0=선점, 1=비선점)
	...
	TEAM(Process process[], int processNum, int processorNum, int preemptive)
	{
	...
	void scheduling()
	{
		...
		for(int currentTime=0;currentTime<burstSum;currentTime++) // 총 실행시간만큼 반복
		{
			...
			for(int j=0;j<tempProcessNumber-1;j++)
			{
				...
				{
					//위험정도가 더 높은 환자부터 치료함[SRTN와 비슷하게 남은 오염도(BurstTime 잔여도)가 더 높은 환자부터 치료한다.]
					if(tempProcess[j].burstTime-tempProcess[j].realBurstTime<tempProcess[k].burstTime-tempProcess[k].realBurstTime)
					{
				...
				System.out.println(currentTime+"초에"+j+"번째 우선순위"+tempProcess[j].processOrder);
			}
			...
			for(int processor=0;processor<processorNum;processor++) // 사용할 프로세서 개수만큼 반복
			{
				...
				if(priority[processor]!=null) // 우선순위로 할당된 프로세스가 종료되었건 아니건 할당만 되어있으면 작동하는 작업
				{
					...
					if(preemptive==0) //선점 스케줄링 정책 채택 시
					{
						...
						else // 우선순위로 채택된 프로세스가 하나의 1작업 후에도 작업이 종료되지 않았을 경우
						{
							priority[processor].use=0;

...
public class RR {
	...
	RR(Process process[], int processNum, int processorNum, int quantum)
	{
		...
		{			
			for(int i=0;i<processNum;i++) //AT가 current time보다 작거나 같아지거나 혹은 작업이 끝나지 않은 프로세스를 준비큐에 삽입
			{
				if(process[i].use==0 && process[i].realBurstTime!=process[i].burstTime && process[i].arrivalTime==currentTime)
				{
					...
					System.out.println(currentTime+"초 프로세스추가");
				}
				System.out.println(i+"프로세스real burst:"+process[i].realBurstTime);
				System.out.println(i+"프로세스 quantum:"+process[i].RRTimeCount);
			}
			...
			for(int processor=0;processor<processorNum;processor++) // 사용할 프로세서 개수만큼 반복
			{
				...
				if(priority[processor]!=null) // 우선순위로 할당된 프로세스가 종료되었건 아니건 할당만 되어있으면 작동하는 작업
				{
					...
					if(priority[processor].use==1) {++priority[processor].realBurstTime;++priority[processor].RRTimeCount;} //해당 프로세서에 할당된 우선순위 프로세스 작업량 1증가시킴
					...
					else if(priority[processor].RRTimeCount==quantum && priority[processor].realBurstTime!=priority[processor].burstTime && priority[processor].use==1)
					{
						priority[processor].RRTimeCount=0;
						priority[processor].use=0;
						//processQueue.offer(priority[processor]); // 준비큐에 해당 프로세스 인큐
						tempQueue.offer(priority[processor]);
						--priority[processor].waitingTime;
						priority[processor]=null;
						System.out.println("나온 프로세스는 이거다 = " +priority[processor]);
					}
				}
			}
			...
			/*for(int z=0;z<processorNum;z++)
			{
				if(priority[z]!=null && priority[z].RRTimeCount==0)
				{
					priority[z].use=1;
				}
			}*/
